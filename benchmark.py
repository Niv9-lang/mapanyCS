...
try:
    ...
    PSUTIL_OK = False
    print("[WARN] psutil not installed - CPU/RAM metrics disabled. Run: pip install psutil")
...
try:
    ...
    MPL_OK = False
    print("[WARN] matplotlib not installed - graph generation disabled. Run: pip install matplotlib")
...
def load_or_create_config(config_path: Path, device_label: str) -> dict:
    ...
    if config_path.exists():
        ...
        if cfg.get("device_label") == device_label:
            print(f"[OK] Machine config loaded: {config_path}")
            return cfg
    ...
    print(f"[OK] Machine config saved: {config_path}")
    return cfg


# ── Monitoring des ressources ───────────────────────────────────────────────
class ResourceMonitor:
    """Surveille CPU et RAM du processus benchmarké (polling 0.5 s)."""

    def __init__(self, target_pid=None, interval=0.5):
        self.target_pid = target_pid
        self.interval = interval
        self._stop = threading.Event()
        self.cpu_samples = []
        self.ram_samples = []   # en Go
        self.scope = "process_tree" if target_pid is not None else "system"
    ...
    def _sample_process_tree(self):
        ...
        for process in processes:
            try:
                ...
                ram_total += process.memory_info().rss
                alive = True
            except (psutil.NoSuchProcess, psutil.AccessDenied):
                continue
        ...
        return cpu_total, ram_total / 1024 ** 3
        ...
        while not self._stop.is_set():
            if PSUTIL_OK:
                if self.target_pid is None:
                    ...
                    cpu_pct, ram_gb = self._sample_process_tree()
...
def make_image_subset(source_dir: Path, images: list[str], tmp_root: Path | None = None) -> Path:
    """
    ...
    """
    if tmp_root is None:
        tmp_root = Path.cwd() / "_benchmark_tmp"
    tmp_root.mkdir(parents=True, exist_ok=True)
    tmp = tmp_root / f"subset_{len(images)}_{int(time.time() * 1000)}"
...
def safe_unlink(path: Path, retries: int = 10, delay_s: float = 0.5) -> bool:
    """Supprime un fichier en tolérant les verrous temporaires Windows."""
...
def run_reconstruction_step(image_folder: Path, output_ply: Path) -> dict:
    ...
    monitor = ResourceMonitor(target_pid=process.pid)
    monitor.start()
...
def generate_graph(results_dir: Path):
    if not MPL_OK:
        print("[WARN] matplotlib unavailable - graph not generated.")
        return

    all_file = results_dir / "all_results.json"
    if not all_file.exists():
        print("[WARN] all_results.json not found.")
        return
    ...
    if not all_runs:
        print("[WARN] all_results.json is empty.")
        return
...
def main():
    ...
    # ── Mode graphe seul ────────────────────────────────────────────────────
    if args.compare:
        print("[INFO] Generating comparison graph...")
    ...
    if not image_folder:
        print("[INPUT] Which photo folder should be used for the benchmark?")
        image_folder = input("   > Path: ").strip().strip('"').strip("'")
    ...
    if not all_images:
        ...
        sys.exit(1)
    print(f"   [OK] {len(all_images)} image(s) found")

    # ── Nom de l'appareil ───────────────────────────────────────────────────
    device_label = args.device_label
    if not device_label:
        print("\n[INPUT] Device label for the graph?")
    ...
    print(f"  Mesures  : CPU/RAM du processus de reconstruction")
    ...
    tmp_root = results_dir / "_tmp_work"
    tmp_root.mkdir(parents=True, exist_ok=True)
    ...
    for n in paliers:
        ...
        ply_out    = subset_dir / "recon_tmp.ply"

        metrics = run_reconstruction_step(subset_dir, ply_out)

        # Supprimer le PLY temporaire immédiatement après la mesure
        if ply_out.exists() and not safe_unlink(ply_out):
            print(f"[WARN] Could not delete temporary file yet: {ply_out}")
        ...
        step_data = {
            ...
            "resource_scope": metrics["resource_scope"],
        }
    ...
    print("[OK] all_results.json updated")

    # ── Graphe final ─────────────────────────────────────────────────────────
    generate_graph(results_dir)
    print(f"\n[OK] Benchmark finished - {len(paliers)} steps completed.\n")
